...
function lookupScprs(){
 ...
 fetch('/api/scprs/{{rid}}').then(r=>r.json()).then(d=>{
  ...
  if(found>0){
   ...
   // Show debug info
   let info=d.results?.map((r,i)=>`Item ${i+1}: ${r.note||r.error||'no data'} (searched: ${(r.searched||[]).join(', ')})`).join('\n');
   if(info)alert('SCPRS Lookup Results:\n\n'+info);
   setTimeout(()=>{btn.textContent='🔍 SCPRS Lookup'},3000);
  }
 }).catch(e=>{btn.disabled=false;btn.textContent='❌ Lookup failed';alert('Error: '+e)});
...
@auth_required
def api_scprs_test():
    """Quick SCPRS search test — pass ?q=stryker+xpr to test."""
    q = request.args.get("q", "restraint strap")
    try:
        from scprs_lookup import _get_session, _build_search_terms
        session = _get_session()
        if not session.initialized:
            session.init_session()
        results = session.search(description=q)
        # Clean out the huge HTML from results before returning
        clean = []
        for r in results:
        ...
        return jsonify({
            ...
            "icsid": session.icsid[:20] + "..." if session.icsid else None,
...
@auth_required
def api_scprs_raw():
    """Raw SCPRS debug — shows actual HTML response from FI$Cal search."""
    q = request.args.get("q", "stryker xpr")
    try:
        from scprs_lookup import (SCPRS_SEARCH_URL, SEARCH_BUTTON,
            FIELD_DESCRIPTION, FIELD_DEPT, FIELD_PO_NUM, FIELD_ACQ_TYPE,
            FIELD_SUPPLIER_ID, FIELD_SUPPLIER_NAME, FIELD_ACQ_METHOD,
            FIELD_FROM_DATE, FIELD_TO_DATE, _get_session)
        ...
        r1 = session.session.get(url, timeout=15)
        page1 = r1.text
        
        # Double-load if needed
        if "ZZ_SCPRS" not in page1:
            import time; time.sleep(0.5)
            r1 = session.session.get(url, timeout=15)
            page1 = r1.text
        
        icsid = session._extract_icsid(page1)
        state_num = session._extract_state_num(page1)
        
        # Step 2: POST search
        form_data = session._build_form_data(page1, SEARCH_BUTTON, {
            FIELD_DESCRIPTION: q,
            FIELD_DEPT: "",
            FIELD_PO_NUM: "",
            FIELD_ACQ_TYPE: "",
        ...
        r2 = session.session.post(SCPRS_SEARCH_URL, data=form_data, timeout=30)
        page2 = r2.text
        
        import re
        # Check what we got back
        has_grid = "PSLEVEL1GRID" in page2
        has_results = "Search Results" in page2
        has_no_match = "No matching" in page2
        title = re.search(r'<title>([^<]*)</title>', page2)
        count = re.search(r'(\d+)\s+to\s+(\d+)\s+of\s+(\d+)', page2)
        ...
        return jsonify({
            ...
            "step1_search_page": {
                ...
                "size": len(page1),
                "has_form": "ZZ_SCPRS" in page1,
                "icsid_found": bool(icsid),
                "state_num": state_num,
            },
            "step2_search_post": {
                ...
                "has_grid": has_grid,
                "has_results_text": has_results,
                "has_no_match": has_no_match,
                "result_count": count.group(0) if count else "not found",
                "post_url": SCPRS_SEARCH_URL,
                "form_action_sent": SEARCH_BUTTON,
            },
            "form_data_sent": {k: v for k, v in form_data.items() if k in 
                [FIELD_DESCRIPTION, "ICAction", "ICStateNum", "ICElementNum", "ICType"]},
            "response_snippet_first500": page2[:500],
            "response_snippet_last500": page2[-500:],
        })
    except Exception as e:
        import traceback
        return jsonify({"error": str(e), "traceback": traceback.format_exc()})
