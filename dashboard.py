...
def do_poll_check():
    ...
    if _shared_poller is None:
        ...
        log.info(f"Created poller for {email_cfg.get('email', 'NO EMAIL SET')}")
...
def generate(rid):
    ...
    try:
        ...
        save_rfqs(rfqs)
        flash(f"Bid package generated for #{sol} — draft email ready", "success")
    except Exception as e:
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                status2, unseen = mail.search(None, "UNSEEN")
                unseen_count = len(unseen[0].split()) if status2 == "OK" and unseen[0] else 0
                diag["inbox_test"] = {
                    "total_emails": total,
                    "unread_emails": unseen_count,
                }
                
                # Show subjects of unread emails
                if unseen_count > 0:
                    subjects = []
                    for msg_id in unseen[0].split()[:5]:
                        st, data = mail.fetch(msg_id, "(BODY[HEADER.FIELDS (SUBJECT FROM)])")
                        if st == "OK":
                            subjects.append(data[0][1].decode("utf-8", errors="replace").strip())
                    diag["inbox_test"]["unread_subjects"] = subjects
