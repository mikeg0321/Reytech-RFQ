...
<div class="card">
 ...
 <div class="bg">
  ...
  <button type="submit" formaction="/rfq/{{rid}}/generate" class="btn btn-g">📄 Generate Reytech Quote</button>
 </div>
 </form>
</div>
...
def pricecheck_detail(pcid):
    ...
    <div class="card">
     ...
     <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      ...
      <button class="btn btn-g" onclick="saveAndGenerate(this)">📄 Generate Reytech Quote</button>
      {download_html}
      <span style="border-left:2px solid #30363d;height:28px;margin:0 8px"></span>
    ...
    function saveAndGenerate(btn) {{
     ...
     showMsg('Saving prices and generating Reytech Quote...','warn');
     ...
     .then(r=>r.json()).then(d=>{{
      if(!d.ok){{btn.textContent='📄 Generate Reytech Quote';btn.disabled=false;showMsg('❌ Save failed','err');return;}}
      btn.textContent='⏳ Generating PDF...';
      return fetch('/pricecheck/{pcid}/generate');
     }}).then(r=>r.json()).then(d=>{{
      btn.disabled=false;btn.textContent='📄 Generate Reytech Quote';
      if(d&&d.ok){{showMsg('✅ Quote generated! Prices saved to Won Quotes KB. Reloading...','ok');setTimeout(()=>location.reload(),1200)}}
      else{{showMsg('❌ Generation failed: '+(d?.error||'unknown'),'err')}}
     }}).catch(e=>{{btn.textContent='📄 Generate Reytech Quote';btn.disabled=false;showMsg('❌ Error: '+e,'err')}});
    }}
