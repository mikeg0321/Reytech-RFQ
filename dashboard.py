...
def pricecheck_detail(pcid):
    ...
    for idx, item in enumerate(items):
        ...
        # Best available cost
        unit_cost = amazon_cost or scprs_cost or 0
        ...
        items_html += f"""<tr>
         ...
         <td>{item.get('qty','')}</td>
         <td>{item.get('uom','')}</td>
         <td style="max-width:250px;font-size:11px">{item.get('description','')[:80]}</td>
    ...
    <div class="card">
     ...
     <table>
      <tr><th>#</th><th>Qty</th><th>UOM</th><th>Description</th><th>SCPRS $</th><th>Amazon $</th><th>Amazon Match</th><th>Unit Cost</th><th>Markup</th><th>Our Price</th><th>Extension</th><th>Conf</th></tr>
      {items_html}
     </table>
    ...
    function recalcPC() {{
     ...
     document.querySelectorAll('input[name^=price_]').forEach((inp,i)=>{{
      ...
      let qtyCell=document.querySelectorAll('td:nth-child(2)');
      let qty=parseInt(qtyCell[i+1]?.textContent)||1;
      let ext=p*qty;
      sub+=ext;
...
def pricecheck_save_prices(pcid):
    ...
    for key, val in data.items():
        try:
            parts = key.split("_")
            field_type = parts[0]  # "price", "cost", or "markup"
            idx = int(parts[1])
            if 0 <= idx < len(items):
