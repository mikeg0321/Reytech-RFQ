...
<div class="card">
 ...
 <a href="/pricecheck/{{id}}" class="rfq-i">
  ...
  <div class="mono">{{pc.items|length}} items</div>
...
def pricecheck_detail(pcid):
    ...
    for idx, item in enumerate(items):
        p = item.get("pricing", {})
        amazon = f"${p['amazon_price']:.2f}" if p.get("amazon_price") else "—"
        scprs = f"${p['scprs_price']:.2f}" if p.get("scprs_price") else "—"
        rec = p.get("recommended_price") or p.get("amazon_price") or 0
        rec_str = f"{rec:.2f}" if rec else ""
        title = (p.get("amazon_title") or "")[:45]
        url = p.get("amazon_url", "")
        link = f'<a href="{url}" target="_blank" title="{p.get("amazon_title","")}">{title}</a>' if url and title else title or "—"
        qty = item.get("qty", 1)
        ext = f"${rec * qty:.2f}" if rec else "—"

        items_html += f"""<tr>
         <td>{item.get('item_number','')}</td>
         <td>{item.get('qty','')}</td>
         <td>{item.get('uom','')}</td>
         <td style="max-width:280px">{item.get('description','')[:80]}</td>
         <td>{scprs}</td>
         <td>{amazon}</td>
         <td>{link}</td>
         <td><input type="number" step="0.01" min="0" name="price_{idx}" value="{rec_str}" style="width:80px;background:#0d1117;border:1px solid #30363d;color:#c9d1d9;padding:4px;border-radius:4px" onchange="recalcPC()"></td>
         <td class="ext">{ext}</td>
        </tr>"""
    ...
    <style>
     ...
     table{{width:100%;border-collapse:collapse}} th,td{{padding:6px 8px;border:1px solid #30363d;text-align:left;font-size:12px}}
     th{{background:#21262d;font-size:11px;text-transform:uppercase;color:#8b949e}}
     .btn{{padding:8px 16px;border-radius:6px;border:none;cursor:pointer;font-weight:600;text-decoration:none;display:inline-block;margin:4px}}
     .btn-p{{background:#1f6feb;color:#fff}} .btn-g{{background:#238636;color:#fff}} .btn-o{{background:#da3633;color:#fff}}
     .btn-y{{background:#9e6a03;color:#fff}} .btn-sm{{padding:5px 12px;font-size:12px}}
     a{{color:#58a6ff}} h1{{margin:0;font-size:22px}} .meta{{color:#8b949e;font-size:13px}}
     .status{{padding:3px 8px;border-radius:4px;font-size:12px;font-weight:600}}
     .status-parsed{{background:#1f6feb33;color:#58a6ff}} .status-priced{{background:#23863633;color:#3fb950}} .status-completed{{background:#23863633;color:#3fb950}}
     .totals{{text-align:right;font-size:14px;margin-top:12px}} .totals b{{color:#3fb950}}
    ...
    <div class="card">
     ...
     <div style="margin-top:12px">
      ...
      <button class="btn btn-g" onclick="saveAndGenerate(this)">📄 Generate Completed PDF</button>
      {download_html}
     </div>
    </div>

    <div class="card">
     <h3 style="margin-top:0">Line Items ({len(items)})</h3>
     <table>
      <tr><th>#</th><th>Qty</th><th>UOM</th><th>Description</th><th>SCPRS $</th><th>Amazon $</th><th>Amazon Match</th><th>Our Price</th><th>Extension</th></tr>
      {items_html}
     </table>
    ...
    function recalcPC() {{
     ...
     document.querySelectorAll('input[name^=price_]').forEach((inp,i)=>{{
      ...
      let qty=parseInt(document.querySelectorAll('td:nth-child(2)')[i+1]?.textContent)||1;
      let ext=p*qty;
      sub+=ext;
      let extCells=document.querySelectorAll('.ext');
      if(extCells[i]) extCells[i].textContent=ext>0?'$'+ext.toFixed(2):'—';
     }});
     document.getElementById('totals').innerHTML='<b>Subtotal: $'+sub.toFixed(2)+'</b>';
    }}
    recalcPC();
    ...
    function runScprs(btn) {{
     ...
     fetch('/pricecheck/{pcid}/scprs-lookup').then(r=>r.json()).then(d=>{{
      if(d.ok){{btn.textContent='✅ '+d.found+' SCPRS matches';setTimeout(()=>location.reload(),1000)}}
    ...
    function runLookup(btn) {{
     ...
     fetch('/pricecheck/{pcid}/lookup').then(r=>r.json()).then(d=>{{
      if(d.ok){{btn.textContent='✅ '+d.found+' prices found';setTimeout(()=>location.reload(),1000)}}
      else{{btn.textContent='❌ '+(d.error||'failed');btn.disabled=false}}
     }}).catch(e=>{{btn.textContent='❌ Error';btn.disabled=false}});
    }}
    ...
    function saveAndGenerate(btn) {{
     ...
     // Collect edited prices
     let prices={{}};
     document.querySelectorAll('input[name^=price_]').forEach(inp=>{{
      prices[inp.name]=parseFloat(inp.value)||0;
     }});
     fetch('/pricecheck/{pcid}/save-prices',{{method:'POST',headers:{{'Content-Type':'application/json'}},body:JSON.stringify(prices)}})
     .then(r=>r.json()).then(d=>{{
      if(!d.ok){{btn.textContent='❌ Save failed';btn.disabled=false;return;}}
      btn.textContent='⏳ Generating PDF...';
      return fetch('/pricecheck/{pcid}/generate');
     }}).then(r=>r.json()).then(d=>{{
      if(d.ok){{btn.textContent='✅ PDF ready';setTimeout(()=>location.reload(),1000)}}
      else{{btn.textContent='❌ '+(d.error||'failed');btn.disabled=false}}
     }}).catch(e=>{{btn.textContent='❌ Error';btn.disabled=false}});
    }}
    </script>
...
def pricecheck_save_prices(pcid):
    """Save manually edited prices from the UI."""
    pcs = _load_price_checks()
    pc = pcs.get(pcid)
    if not pc:
        return jsonify({"ok": False, "error": "PC not found"})
    ...
    for key, val in prices.items():
        # key is "price_0", "price_1", etc.
        try:
            idx = int(key.replace("price_", ""))
            if 0 <= idx < len(items):
...
def pricecheck_generate(pcid):
    ...
    output_path = os.path.join(DATA_DIR, f"PC_{safe_name}_completed.pdf")
