...
<div class="card">
 ...
 <div class="bg">
  ...
  <button type="button" class="btn" style="background:var(--sf2);color:var(--tx);border:1px solid var(--bd)" onclick="showRfqPreview()">👁️ Preview Quote</button>
  <button type="submit" formaction="/rfq/{{rid}}/generate" class="btn btn-g">📄 Generate State Forms (704B + Package)</button>
  <a href="/rfq/{{rid}}/generate-quote" class="btn" style="background:#1a3a5c;color:#fff">📋 Generate Reytech Quote</a>
 </div>
 </form>
</div>
...
<div class="card">
 ...
 <form method="POST" action="/rfq/{{rid}}/upload-templates" enctype="multipart/form-data">
  <div style="display:flex;gap:8px;align-items:center">
   <input type="file" name="templates" multiple accept=".pdf" style="font-size:13px">
   <button type="submit" class="btn btn-sm btn-p">📎 Upload Templates</button>
  </div>
...
def pricecheck_detail(pcid):
    ...
    <a href="/" style="color:#58a6ff;font-size:15px">← Dashboard</a>
    ...
    <div class="card">
     ...
     <div style="margin-top:12px;display:flex;gap:6px;flex-wrap:wrap;align-items:center">
      ...
      <button class="btn" style="background:#21262d;color:#c9d1d9;border:1px solid #484f58" onclick="showPreview()">👁️ Preview Quote</button>
      <button class="btn btn-g" onclick="saveAndGenerate(this)">📄 Generate Completed 704</button>
      <button class="btn" style="background:#1a3a5c;color:#fff" onclick="generateReytechQuote(this)">📋 Reytech Quote PDF</button>
      {download_html}
      <span style="border-left:2px solid #30363d;height:28px;margin:0 8px"></span>
    ...
    function showPreview() {{
     ...
     rows.forEach((row,i)=>{{
      ...
      itemsHtml+=`<tr>
       ...
       <td style="text-align:center">${{uom.toUpperCase()}}</td>
     ...
     const html=`<div class="q-preview" style="padding:20px">
      <div class="q-header">
       <div style="display:flex;justify-content:space-between;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:#444">
        <span>STATE OF CALIFORNIA</span><span>CALIFORNIA CORRECTIONAL HEALTH CARE SERVICES</span>
       </div>
       <h3 style="margin:6px 0 2px">PRICE CHECK WORKSHEET — GOODS</h3>
       <small>AMS 704 (Rev 1/2019)</small>
      </div>

      <table style="margin-top:10px">
       <tr><td style="width:30%"><span style="font-size:9px;color:#555">PRICE CHECK #</span><br><b>${{PC_META.pcNum}}</b></td>
           <td colspan="2"><span style="font-size:9px;color:#555">DUE DATE</span><br><b>${{PC_META.dueDate}}</b></td></tr>
       <tr><td><span style="font-size:9px;color:#555">Requestor</span><br>${{PC_META.requestor}}</td>
           <td><span style="font-size:9px;color:#555">Institution</span><br>${{PC_META.institution}}</td>
           <td><span style="font-size:9px;color:#555">Ship to</span><br>${{PC_META.shipTo}}</td></tr>
      </table>

      <div style="background:#e8e8e0;text-align:center;font-weight:700;font-size:12px;padding:4px;margin-top:10px;border:1px solid #000;letter-spacing:2px">SUPPLIER INFORMATION</div>
      <div class="q-supplier">
       <div><span class="q-lbl">Company Name</span><b>${{REYTECH.name}}</b></div>
       <div><span class="q-lbl">Representative</span><b>${{REYTECH.rep}}</b></div>
       <div><span class="q-lbl">Address</span>${{REYTECH.address}}</div>
       <div><span class="q-lbl">Delivery (ARO)</span><b>${{delivery}}</b></div>
       <div><span class="q-lbl">Phone</span>${{REYTECH.phone}}</div>
       <div><span class="q-lbl">Discount Offered</span>Included</div>
       <div><span class="q-lbl">E-Mail Address</span>${{REYTECH.email}}</div>
       <div><span class="q-lbl">Price Check Expires</span><b>${{document.getElementById('expiryDate')?.textContent||''}}</b></div>
       <div><span class="q-lbl">Certified SB/MB #</span>${{REYTECH.sbmb}}</div>
       <div><span class="q-lbl">Certified DVBE #</span>${{REYTECH.dvbe}}</div>
      </div>

      <table style="margin-top:12px">
       <thead><tr>
        <th style="width:40px">Item #</th><th style="width:40px">Qty</th><th style="width:40px">UOM</th><th style="width:40px">Qty/UOM</th>
        <th>Item Description</th><th style="width:120px">Substituted Item</th>
        <th style="width:70px;text-align:right">Price/Unit</th><th style="width:80px;text-align:right">Extension</th>
       </tr></thead>
       <tbody>${{itemsHtml}}</tbody>
      </table>

      <div style="display:flex;justify-content:space-between;margin-top:8px">
       <div style="flex:1;font-size:12px;border:1px solid #000;padding:6px 8px">
        <span style="font-size:9px;color:#555;text-transform:uppercase">Supplier and/or Requestor Notes:</span><br>
        ${{notes||'<span style="color:#999">—</span>'}}
       </div>
       <div style="min-width:200px;text-align:right">
        <table style="margin-left:auto">
         <tr><td style="text-align:right;font-size:12px;border:1px solid #000;padding:3px 8px">Merchandise Subtotal</td>
             <td style="text-align:right;font-size:13px;font-weight:700;border:1px solid #000;padding:3px 8px">$ ${{subtotal.toFixed(2)}}</td></tr>
         <tr><td style="text-align:right;font-size:12px;border:1px solid #000;padding:3px 8px">Freight</td>
             <td style="text-align:right;font-size:13px;border:1px solid #000;padding:3px 8px">$ 0.00</td></tr>
         <tr><td style="text-align:right;font-size:12px;border:1px solid #000;padding:3px 8px">Tax</td>
             <td style="text-align:right;font-size:13px;border:1px solid #000;padding:3px 8px">$ ${{tax.toFixed(2)}}</td></tr>
         <tr><td style="text-align:right;font-size:13px;font-weight:700;border:2px solid #000;padding:4px 8px">TOTAL PRICE</td>
             <td style="text-align:right;font-size:14px;font-weight:700;border:2px solid #000;padding:4px 8px">$ ${{total.toFixed(2)}}</td></tr>
        </table>
       </div>
      </div>

      <div style="margin-top:12px;font-size:10px;text-align:center;color:#555;border-top:1px solid #999;padding-top:6px">
       THIS DOCUMENT IS USED FOR INFORMATIONAL PURPOSES ONLY. IT IS <b>NOT</b> AN ORDER.
      </div>
     </div>`;
...
@auth_required
def quotes_list():
    """Browse / search all generated Reytech quotes."""
    if not QUOTE_GEN_AVAILABLE:
        flash("Quote generator not available", "error")
        return redirect("/")
    q = request.args.get("q", "")
    agency_filter = request.args.get("agency", "")
    quotes = search_quotes(query=q, agency=agency_filter, limit=100)
    ...
    for qt in quotes:
        ...
        dl = f'<a href="/api/pricecheck/download/{fname}">📥</a>' if fname else ""
        rows_html += f"""<tr>
         <td style="font-weight:600">{qt.get('quote_number','')}</td>
         <td>{qt.get('date','')}</td>
         <td>{qt.get('agency','')}</td>
         <td>{qt.get('institution','')[:40]}</td>
         <td>{qt.get('rfq_number','')}</td>
         <td style="text-align:right;font-weight:600">${qt.get('total',0):,.2f}</td>
         <td style="text-align:center">{qt.get('items_count',0)}</td>
    ...
    return render(f"""
     ...
     <div class="card" style="margin-bottom:16px;padding:16px">
      <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
       <form method="get" style="display:flex;gap:8px;flex:1;min-width:200px">
        <input name="q" value="{q}" placeholder="Search quotes..." style="flex:1;padding:8px;background:var(--sf);border:1px solid var(--bd);border-radius:6px;color:var(--tx)">
       ...
       <span style="color:var(--tx2)">Next: <strong>{next_num}</strong></span>
      </div>
     </div>
     <div class="card" style="margin-bottom:16px;padding:16px">
      <div style="display:flex;gap:16px;align-items:center;flex-wrap:wrap">
       <span>Logo: {"✅ Uploaded" if logo_exists else "❌ Not uploaded (using text fallback)"}</span>
     ...
     <div class="card" style="padding:0;overflow-x:auto">
      <table>
       <thead><tr>
        <th>Quote #</th><th>Date</th><th>Agency</th><th>Institution</th><th>RFQ #</th><th style="text-align:right">Total</th><th>Items</th><th>PDF</th>
       </tr></thead>
       <tbody>{rows_html if rows_html else '<tr><td colspan="8" style="text-align:center;padding:24px;color:var(--tx2)">No quotes yet — generate your first from a Price Check or RFQ</td></tr>'}</tbody>
      </table>
     </div>
