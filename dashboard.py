...
def pricecheck_detail(pcid):
    ...
    for item in items:
        ...
        rec = f"${p['recommended_price']:.2f}" if p.get("recommended_price") else "—"
        title = (p.get("amazon_title") or "")[:50]
    ...
    <style>
     ...
     table{{width:100%;border-collapse:collapse}} th,td{{padding:6px 10px;border:1px solid #30363d;text-align:left;font-size:13px}}
     th{{background:#21262d}} .btn{{padding:8px 16px;border-radius:6px;border:none;cursor:pointer;font-weight:600;text-decoration:none;display:inline-block;margin:4px}}
     .btn-p{{background:#1f6feb;color:#fff}} .btn-g{{background:#238636;color:#fff}} .btn-o{{background:#da3633;color:#fff}}
     a{{color:#58a6ff}} h1{{margin:0}} .meta{{color:#8b949e;font-size:13px}}
    ...
    <div class="card">
     ...
     <div style="margin-top:12px">
      <button class="btn btn-p" onclick="runLookup()">🔬 Amazon Lookup</button>
      <button class="btn btn-g" onclick="generatePC()">📄 Generate Completed PDF</button>
      {download_html}
     </div>
    </div>

    <div class="card">
     <h3>Line Items ({len(items)})</h3>
     <table>
      <tr><th>#</th><th>Qty</th><th>UOM</th><th>Description</th><th>Amazon $</th><th>Match</th><th>Our Price</th></tr>
      {items_html}
     </table>
    </div>
    ...
    function generatePC() {{
     const btn=event.target;btn.disabled=true;btn.textContent='⏳ Generating PDF...';
     fetch('/pricecheck/{pcid}/generate').then(r=>r.json()).then(d=>{{
...
def pricecheck_generate(pcid):
    """Generate completed Price Check PDF."""
    if not PRICE_CHECK_AVAILABLE:
        return jsonify({"ok": False, "error": "price_check.py not available"})
    pcs = _load_price_checks()
    pc = pcs.get(pcid)
