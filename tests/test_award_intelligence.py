...
Originally written as a standalone script with top-level asserts and
`sys.exit(1)` on failure — which crashed pytest collection (SystemExit
at import time). Wrapped in a single pytest function so the assertions
run under both pytest AND `python tests/test_award_intelligence.py`.
"""
...
def test_award_intelligence_audit():
    errors = 0
    tests = 0
    failed = []
    ...
    # ── Config ──────────────────────────────────────────────────────────────
    print("-- Config --")
    check("4 update times", len(SCPRS_UPDATE_TIMES_PT) == 4)
    check("Times 7/9:30/12/17",
          SCPRS_UPDATE_TIMES_PT == [dt_time(7, 0), dt_time(9, 30),
                                    dt_time(12, 0), dt_time(17, 0)])
    check("Expiry 45 days", EXPIRY_DAYS == 45)
    check("Daily phase 2 days", DAILY_CHECK_PHASE_DAYS == 2)
    check("Max per run 20", MAX_SCPRS_PER_RUN == 20)
    ...
    check("2d = daily (boundary)", get_check_phase(datetime(2026, 3, 25, 10, 0), datetime(2026, 3, 27, 10, 0)) == "daily")
    check("4d = intensive (post DAILY_CHECK_PHASE_DAYS=2 tightening)",
          get_check_phase(datetime(2026, 3, 23, 10, 0), datetime(2026, 3, 27, 10, 0)) == "intensive")
    ...
    check("19 OK", check_rate_limit(19)[0])
    check("20 BLOCKED (MAX_SCPRS_PER_RUN)", not check_rate_limit(20)[0])
    ...
    # Post-9:30-window addition: 8am → next window is 9:30 (not noon).
    check("Intensive returns 9:30 today",
          nct2 is not None and nct2.hour == 9 and nct2.minute == 30)
    ...
    assert errors == 0, f"{errors} award-intelligence checks failed: {failed}"


if __name__ == "__main__":
    test_award_intelligence_audit()
