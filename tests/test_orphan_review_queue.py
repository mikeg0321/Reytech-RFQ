...
class TestFuzzyCandidateScoring:
    ...
    # DELETED 2026-05-27 (Job #1): test_institution_named_quote_resolves_to_parent_agency
    #
    # This test pinned the CCHCS-specific canonicalization path: a CDCR
    # institution name in the quotes.agency column ("CSP California State
    # Prison - Sacramento") would resolve to the same canonical key as
    # the order's "CCHCS" code, via `_agency_canonical()` → `match_agency()`
    # → the DEFAULT_AGENCY_CONFIGS["cchcs"] match_patterns list.
    #
    # The CCHCS dict was DELETED per §0 LAW 2 (Spine is canonical).
    # `match_agency` now returns "other" for both sides, and they remain
    # unmatched at the high tier — `_agency_canonical` falls through to
    # `raw.lower()` for the institution name. The mechanism the test was
    # asserting (parent_agency canonicalization via the legacy dict) no
    # longer exists.
    #
    # The general canonicalization invariant (lookup → key → match) is
    # still covered by the surviving DSH/CalVet tests in
    # tests/test_review_alignment.py and tests/test_facility_registry.py.
    # For CCHCS, this exact canonicalization is intentionally retired;
    # orphan-linking for CCHCS-class orders runs through the Spine path.
