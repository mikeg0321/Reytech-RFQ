...
# ═══════════════════════════════════════════════════════════════════════════════
# QUOTE NUMBERING — R{YY}Q{seq}, resets Jan 1 each year
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# QUOTES DATABASE — searchable log of all generated quotes
# ═══════════════════════════════════════════════════════════════════════════════
...
def search_quotes(query: str = "", agency: str = "", limit: int = 50) -> list:
    """Search quotes by number, institution, rfq number, or agency."""
...
def generate_quote(
    ...
    if logo_path:
        try:
            ...
            # Position: top-origin y=152, so rl_y = Y(152+dh)
            logo_rl_y = Y(152) - dh
            c.drawImage(logo_path, ML + 30, logo_rl_y, width=dw, height=dh,
    ...
    if not logo_path:
        # Text-based logo — navy chevron + company name
        p = c.beginPath()
        bx, by = ML + 8, Y(164)
        p.moveTo(bx, by)
        ...
        c.setFillColor(NAVY)
        c.drawString(ML + 34, Y(166), "Reytech Inc.")
...
def generate_quote_from_pc(pc: dict, output_path: str, **kwargs) -> dict:
    ...
    data = {
        ...
        "ship_to_name": (pc.get("ship_to", "") or "").split(",")[0],
        "ship_to_address": [pc.get("ship_to", "")] if pc.get("ship_to") else [],
...
def generate_quote_from_rfq(rfq: dict, output_path: str, **kwargs) -> dict:
    ...
    data = {
        ...
        "ship_to_name": (rfq.get("ship_to", rfq.get("delivery_location", "")) or "").split(",")[0],
        "ship_to_address": [rfq.get("ship_to", rfq.get("delivery_location", ""))],
