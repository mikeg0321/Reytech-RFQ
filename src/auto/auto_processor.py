...
def auto_process_price_check(pdf_path: str, pc_id: str = None) -> dict:
    ...
    log.info("Auto-processing PC: %s (pdf=%s)", pc_id or "new", pdf_path)
    result = {
        "type": "price_check",
        "pc_id": pc_id,
        "steps": [],
    ...
    log.info("Auto-process complete: %s — %d items, grade=%s, %.1fs",
             pc_id or "new", len(items),
             confidence.get("overall_grade", "?"), elapsed)
