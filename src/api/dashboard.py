...
def process_rfq_email(rfq_email):
    ...
    try:
        ...
        if classify_enabled():
            _v2_atts = rfq_email.get("attachments", [])
            _v2_files = [
 ...
 <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px">
  <div style="min-width:0;flex:1">
   <div class="card-t" style="margin:0;display:flex;align-items:center;gap:8px;cursor:pointer" onclick="toggleBriefBody()">
    &#x1F9E0; Manager Brief
    <span id="brief-badge" style="font-size:13px;padding:2px 8px;border-radius:10px;background:var(--r-surface-2);color:var(--r-text-muted);font-weight:500"></span>
    <span id="brief-toggle" style="font-size:13px;color:var(--r-text-muted);margin-left:4px">&#x25BC;</span>
   </div>
  ...
  <div style="display:flex;gap:6px;align-items:center;flex-shrink:0">
   ...
   <button onclick="loadBrief(true)" id="brief-refresh-btn" style="font-size:13px;padding:4px 8px;background:rgba(79,140,255,.1);border:1px solid rgba(79,140,255,.3);color:var(--r-accent);border-radius:6px;cursor:pointer;white-space:nowrap">&#x1F504; Refresh</button>
  </div>
 </div>
 <div id="brief-body">
  <div id="brief-grid">
   <div style="font-size:14px;font-weight:600;color:var(--r-text-muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:10px;display:flex;align-items:center;gap:6px">
    Needs Your Attention <span id="approval-count" class="brief-count"></span>
   </div>
   <div id="approvals-list"></div>
  </div>
  <div id="pipeline-bar" style="display:flex;gap:12px;flex-wrap:wrap;margin-top:16px;padding-top:14px;border-top:1px solid var(--r-border)"></div>
  <div id="agents-row" style="display:none;margin-top:16px;padding-top:14px;border-top:1px solid var(--r-border)">
   <div style="font-size:13px;font-weight:700;color:var(--r-text-muted);text-transform:uppercase;letter-spacing:1px;margin-bottom:10px">Agent Status</div>
...
function renderBrief(data){
 ...
 if(bar){var s=data.summary||{};var q=s.quotes||{};var gr=s.growth||{};var ob=s.outbox||{};var rv=data.revenue||{};var ag=data.agents_summary||{};var stats=[{label:'Quotes',value:q.total||0,color:'var(--r-accent)'},{label:'Pipeline $',value:'$'+(q.pipeline_value||0).toLocaleString(),color:'var(--r-warn)'},{label:'Won $',value:'$'+(q.won_total||0).toLocaleString(),color:'var(--r-accent)'},{label:'Win Rate',value:(q.win_rate||0)+'%',color:q.win_rate>=50?'var(--r-accent)':'var(--r-warn)'},{label:'Growth',value:gr.total_prospects||0,color:'#bc8cff'},{label:'Agents',value:(ag.healthy||0)+'/'+(ag.total||0),color:ag.down>0?'var(--r-bad)':'var(--r-accent)'},{label:'Goal',value:rv.pct?rv.pct.toFixed(0)+'%':'0%',color:rv.pct>=50?'var(--r-accent)':rv.pct>=25?'var(--r-warn)':'var(--r-bad)'},{label:'Drafts',value:ob.drafts||0,color:ob.drafts>0?'var(--r-warn)':'var(--r-text-muted)'}];bar.innerHTML=stats.map(function(s){return '<div class="stat-chip"><div class="stat-val" style="color:'+s.color+'">'+s.value+'</div><div class="stat-label">'+s.label+'</div></div>';}).join('');}
...
from src.api.config import (INTEL_AVAILABLE, PREDICT_AVAILABLE, QB_AVAILABLE, GROWTH_AVAILABLE,
    OUTREACH_AVAILABLE, VOICE_AVAILABLE, SCANNER_AVAILABLE,
    LEADGEN_AVAILABLE, ITEM_ID_AVAILABLE, REPLY_ANALYZER_AVAILABLE, QA_AVAILABLE,
    MANAGER_AVAILABLE, CATALOG_AVAILABLE, _WF_AVAILABLE)
...
_ROUTE_MODULES = [
    ...
    "routes_kpi",             # KPI Phase A: pipeline funnel + orphan-order triage
    ...
    "routes_cross_sell",           # 2026-05-11 needle-mover #2 Phase 2b: cross-sell intel API + digest preview/send
