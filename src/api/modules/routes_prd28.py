...
<!-- Goal Tracker -->
<div id="goal-section" style="background:var(--crd);border:1px solid var(--r-border);border-radius:12px;padding:20px;margin-bottom:20px">
 <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
  <h3 style="margin:0;font-size:16px">🎯 Annual Goal: $2M</h3>
  <span id="goal-pct" style="font-size:24px;font-weight:700;color:var(--r-accent)">--</span>
 </div>
 <div style="background:var(--r-bg);border-radius:8px;height:24px;overflow:hidden;margin-bottom:12px">
 ...
 <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;text-align:center">
  <div><div style="font-size:14px;color:var(--r-text-muted)">YTD Revenue</div><div id="goal-ytd" style="font-size:18px;font-weight:600">$--</div></div>
  <div><div style="font-size:14px;color:var(--r-text-muted)">Weighted Pipeline</div><div id="goal-pipe" style="font-size:18px;font-weight:600;color:var(--r-warn)">$--</div></div>
  <div><div style="font-size:14px;color:var(--r-text-muted)">Projected Annual</div><div id="goal-proj" style="font-size:18px;font-weight:600">$--</div></div>
  <div><div style="font-size:14px;color:var(--r-text-muted)">Daily Run Rate</div><div id="goal-daily" style="font-size:18px;font-weight:600">$--</div></div>
 </div>
</div>

<!-- Monthly Chart + Pipeline -->
<div style="display:grid;grid-template-columns:2fr 1fr;gap:16px;margin-bottom:20px">
 <div style="background:var(--crd);border:1px solid var(--r-border);border-radius:12px;padding:20px">
  ...
  try {
    ...
    if (pEl) {
      pEl.innerHTML = pipe.length ? pipe.slice(0,10).map(p =>
        `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--r-border);font-size:14px">
          <div><a href="/quote/${p.quote_number}" style="color:var(--r-accent)">${p.quote_number}</a>
           <span style="color:var(--r-text-muted);margin-left:4px">${p.agency||''}</span></div>
          <div style="text-align:right">
            <div style="font-weight:600">$${(p.weighted_value||0).toLocaleString()}</div>
            <div style="font-size:13px;color:var(--r-text-muted)">${(p.win_probability*100).toFixed(0)}% × $${(p.total||0).toLocaleString()}</div>
          </div>
        </div>`
      ).join('') : '<div style="color:var(--r-text-muted);font-size:14px">No open quotes in pipeline</div>';
    }
    ...
    if (mSummary) {
      mSummary.innerHTML = m.total_quotes_with_cost ?
        ...
        </div>` : '<span style="font-size:14px;color:var(--r-text-muted)">No cost data yet</span>';
    }
    const mList = document.getElementById('margin-list');
    if (mList && m.items) {
      mList.innerHTML = m.items.slice(0,10).map(i =>
        `<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--r-border);font-size:14px">
    ...
    if (tcEl) {
      tcEl.innerHTML = tc.length ? tc.map((c,i) =>
        `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid var(--r-border);font-size:14px">
          <span>${i+1}. ${c.agency||'Unknown'} <span style="color:var(--r-text-muted)">(${c.deal_count} deals)</span></span>
          <span style="font-weight:600">$${(c.total_revenue||0).toLocaleString(undefined,{maximumFractionDigits:0})}</span>
        </div>`
      ).join('') : '<div style="color:var(--r-text-muted);font-size:14px">No revenue data yet</div>';
    }
