...
def _pricecheck_detail_inner(pcid):
    ...
    for idx, item in enumerate(items):
        ...
        if asin:
            link_parts.append(f'<span style="color:var(--r-accent);font-size:13px;font-family:JetBrains Mono,monospace">ASIN: {asin}</span>')
        # Web search source
        ...
        link_display = f'<a href="{item_link}" target="_blank" style="font-size:14px;color:var(--r-accent);word-break:break-all">{item_supplier or item_link[:30]}</a>' if item_link else ""
        ...
        # renderer will compute via pricing_math.is_billable. Closes the
        # UI/PDF predicate-divergence finding from the 2026-05-11 audit.
        _no_bid_attr = ' data-no-bid="1"' if no_bid else ''
        items_html += f"""<tr style="{row_opacity}" data-row="{idx}"{_disc_attr}{_no_bid_attr}>
         ...
         <td style="white-space:nowrap"><div style="display:flex;align-items:center;gap:2px"><input type="text" inputmode="numeric" name="markup_{idx}" value="{markup_pct}" class="num-in sm" style="width:68px" oninput="sanitizeInt(this)" onchange="recalcRow({idx},true)"><span style="color:#8b949e;font-size:13px">%</span></div></td>
...
def _pricecheck_generate_quote_locked(pcid):
    ...
    if not locked_qn:
        # Ghost-data gate with auto-synth fallback (parallel to the
        # RFQ-side wrapper). Heals records that pre-date the ingest
        # synthesizer; items=0 / Reytech-internal buyer still block.
        ...
        if _synthed:
            t.step(f"Auto-synthesized pc#: {pc.get('pc_number')}")
        if not _ok:
