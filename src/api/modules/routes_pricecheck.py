...
def _generate_pc_pdf(pcid):
    ...
    # For DOCX/office sources: use the blank AMS 704 template with form fields.
    # LibreOffice conversion produces flat PDFs where the overlay consistently
    # misaligns (3+ attempts failed — coordinates don't match DOCX layout).
    # The blank template has proper Row1..Row11 / Row1_2..Row8_2 form fields
    # that fill_ams704 fills correctly via pypdf form fill (proven path).
    if _is_docx_source or _is_converted_docx:
        ...
        if os.path.exists(_blank_704):
            ...
            return {"ok": False, "error": "Blank 704 template not found — cannot generate from DOCX source."}
