...
def _pricecheck_detail_inner(pcid):
    ...
    for idx, item in enumerate(items):
        ...
        items_html += f"""<tr style="{row_opacity}" data-row="{idx}">
         ...
         <td style="text-align:center;font-weight:600;font-size:13px;color:#8b949e;font-family:'JetBrains Mono',monospace;position:relative">{line_num}{'<button onclick=\"mergeUp('+str(idx)+');event.stopPropagation()\" title=\"Merge into item above (multi-line description fix)\" style=\"position:absolute;top:-2px;right:-2px;background:#21262d;border:1px solid #30363d;border-radius:3px;color:#a78bfa;font-size:10px;cursor:pointer;padding:1px 3px;display:none\" class=\"merge-btn\" aria-label=\"Merge into item above\" onmouseover=\"this.style.background=&apos;#30363d&apos;\" onmouseout=\"this.style.background=&apos;#21262d&apos;\">⬆</button>' if idx > 0 else ''}</td>
