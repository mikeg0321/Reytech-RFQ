...
def generate_rfq_package(rid):
    ...
    if QUOTE_GEN_AVAILABLE:
        try:
            ...
            # Collision detection: if this quote number was generated by a DIFFERENT RFQ, get a new one
            if locked_qn:
                try:
                    from src.core.db import get_db as _gdb
                    with _gdb() as _qconn:
                        existing = _qconn.execute(
                            "SELECT quote_number, total, created_at FROM quotes WHERE quote_number=?",
                            (locked_qn,)
                        ).fetchall()
                        if len(existing) > 1:
                            # Multiple quotes with same number = definite collision
                            t.warn(f"Quote collision: {locked_qn} has {len(existing)} entries, getting new number")
                            locked_qn = ""
                            r["reytech_quote_number"] = ""
                        elif len(existing) == 1:
                            eq = dict(existing[0])
                            # Compare total — if quote total doesn't match RFQ total, it's from a different RFQ
                            rfq_total = sum(
                                (it.get("price_per_unit") or 0) * (it.get("qty") or 1)
                                for it in r.get("line_items", [])
                            )
                            eq_total = eq.get("total") or 0
                            if abs(rfq_total - eq_total) > 1.0 and eq_total > 0:
                                t.warn(f"Quote collision: {locked_qn} total=${eq_total:.2f} but RFQ total=${rfq_total:.2f}, getting new number")
                                locked_qn = ""
                                r["reytech_quote_number"] = ""
...
def rfq_generate_quote(rid):
    ...
    # Collision detection: if this number used by a different quote, get a new one
    if locked_qn:
        try:
            from src.core.db import get_db as _gdb2
            with _gdb2() as _qconn2:
                existing = _qconn2.execute(
                    "SELECT total FROM quotes WHERE quote_number=?", (locked_qn,)
                ).fetchall()
                if len(existing) > 1:
                    log.warning("Quote collision: %s has %d entries, assigning new", locked_qn, len(existing))
                    locked_qn = ""
                    r["reytech_quote_number"] = ""
                elif len(existing) == 1:
                    eq_total = dict(existing[0]).get("total") or 0
                    rfq_total = sum(
                        (it.get("price_per_unit") or 0) * (it.get("qty") or 1)
                        for it in r.get("line_items", [])
                    )
                    if abs(rfq_total - eq_total) > 1.0 and eq_total > 0:
                        log.warning("Quote collision: %s total mismatch ($%.2f vs $%.2f), assigning new", locked_qn, eq_total, rfq_total)
                        locked_qn = ""
                        r["reytech_quote_number"] = ""
        except Exception:
            pass
