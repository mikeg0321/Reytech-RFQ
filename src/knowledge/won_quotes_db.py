...
# ─── Storage Layer (SQLite — Railway-persistent) ─────────────────────────────
...
def _ensure_won_quotes_table():
    ...
    try:
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_wq_item_number ON won_quotes(item_number)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_wq_category    ON won_quotes(category)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_wq_source      ON won_quotes(source)")
        conn.commit()
    finally:
        conn.close()


def load_won_quotes(limit: int = MAX_RECORDS) -> list:
    """Load won quotes from SQLite. Returns list of dicts (same format as before)."""
    try:
        _ensure_won_quotes_table()
        conn = _get_db_conn()
        rows = conn.execute(
            "SELECT * FROM won_quotes ORDER BY ingested_at DESC LIMIT ?", (limit,)
        ).fetchall()
        conn.close()
        records = []
        for row in rows:
            d = dict(row)
            # Deserialize tokens JSON → set (matching original format)
            try:
                d["tokens"] = set(json.loads(d["tokens"] or "[]"))
            except Exception:
                d["tokens"] = set()
            records.append(d)
        log.debug("Loaded %d won quotes from SQLite", len(records))
        return records
    except Exception as e:
        log.error("load_won_quotes SQLite error: %s", e)
        # Fallback: try JSON file
        if os.path.exists(WON_QUOTES_FILE):
            try:
...
def save_won_quotes(quotes: list):
    ...
    Bulk-upserts a list of dicts into SQLite.
    """
    if not quotes:
        return
    try:
        ...
        for q in quotes:
            ...
            """, (
                q.get("id"), q.get("po_number"), q.get("item_number"),
                q.get("description", ""), q.get("normalized_description", ""),
                json.dumps(tokens),
                q.get("category", "general"), q.get("supplier", ""),
                q.get("department", ""), float(q.get("unit_price", 0)),
                float(q.get("quantity", 1)), q.get("total"),
                q.get("award_date", ""), q.get("source", "scprs_live"),
                float(q.get("confidence", 1.0)),
                q.get("ingested_at", now), now,
            ))
        conn.commit()
        conn.close()
        log.debug("save_won_quotes: upserted %d records to SQLite", len(quotes))
    except Exception as e:
        log.error("save_won_quotes SQLite error: %s", e)
...
def token_overlap_score(tokens_a: set, tokens_b: set) -> float:
    """Calculate overlap score between query tokens (a) and document tokens (b).
    
    Uses query-centric recall: what % of query tokens appear in the document.
    This ensures short queries like 'nitrile gloves' match long descriptions
    like 'GLOVES EXAM NITRILE BLK 5 MIL POWER-FREE 100/BX'.
    
    Also blends in a small Jaccard component to reward tighter matches.
    """
    ...
    if not intersection:
        return 0.0
    # Query recall: what fraction of query terms appear in the doc
    query_recall = len(intersection) / len(tokens_a)
    # Jaccard: penalizes very loose matches slightly
    jaccard = len(intersection) / len(tokens_a | tokens_b)
    # Blend: 70% recall (favor short query matching), 30% jaccard (favor precision)
...
def find_similar_items(
    ...
    try:
        ...
        for row in rows:
            d = dict(row)
            try: d["tokens"] = set(json.loads(d["tokens"] or "[]"))
            except Exception: d["tokens"] = set()
            quotes.append(d)
    except Exception as e:
        log.error("find_similar_items DB error: %s — falling back to JSON", e)
        quotes = load_won_quotes()

    if not quotes:
        log.debug("find_similar_items: KB empty, no matches for '%s'", description[:40])
        return []
    ...
    if final:
        log.info("KB match: '%s' → %d results, best=%.1f%% conf",
                 description[:35], len(final), final[0]["match_confidence"] * 100)
    return final
...
def ingest_scprs_result(
    ...
    """
    Ingest a single SCPRS result into the Won Quotes KB (SQLite).
    ...
    try:
        ...
        _rp(
            ...
            notes=f"PO:{po_number} | {supplier}" if supplier else f"PO:{po_number}",
        )
    except Exception as e:
...
def ingest_scprs_bulk(results: list) -> dict:
    ...
    log.info("KB bulk ingest: %d new, %d updated, %d skipped",
...
def get_kb_stats() -> dict:
    """Return statistics about the Won Quotes Knowledge Base (from SQLite)."""
    try:
        _ensure_won_quotes_table()
        conn = _get_db_conn()
        total = conn.execute("SELECT COUNT(*) FROM won_quotes").fetchone()[0]
        if total == 0:
            conn.close()
            return {"total_records": 0, "categories": {}, "departments": {},
                    "suppliers": {}, "date_range": None, "avg_unit_price": None}

        # Aggregate queries — much faster than Python loops over thousands of rows
        cats   = {r[0]: r[1] for r in conn.execute("SELECT category, COUNT(*) FROM won_quotes GROUP BY category").fetchall()}
        depts  = {r[0]: r[1] for r in conn.execute("SELECT department, COUNT(*) FROM won_quotes WHERE department!='' GROUP BY department ORDER BY 2 DESC LIMIT 10").fetchall()}
        sups   = {r[0]: r[1] for r in conn.execute("SELECT supplier, COUNT(*) FROM won_quotes WHERE supplier!='' GROUP BY supplier ORDER BY 2 DESC LIMIT 10").fetchall()}
        prices = conn.execute("SELECT AVG(unit_price), MIN(unit_price), MAX(unit_price) FROM won_quotes WHERE unit_price>0").fetchone()
        dates  = conn.execute("SELECT MIN(award_date), MAX(award_date) FROM won_quotes WHERE award_date!=''").fetchone()
        conn.close()
        ...
        return {
            ...
            "departments":    depts,
            "suppliers":      sups,
            "date_range":     {"earliest": dates[0], "latest": dates[1]} if dates and dates[0] else None,
            "avg_unit_price": round(prices[0], 2) if prices and prices[0] else None,
            "min_price":      round(prices[1], 2) if prices and prices[1] else None,
            "max_price":      round(prices[2], 2) if prices and prices[2] else None,
            "storage":        "sqlite",
        }
    except Exception as e:
