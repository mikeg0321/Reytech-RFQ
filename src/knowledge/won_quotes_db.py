...
def load_won_quotes() -> list:
    ...
    try:
        ...
        log.debug("Loaded %d won quotes from KB", len(records))
        return records
    except (json.JSONDecodeError, IOError) as e:
        log.error("Failed to load won quotes: %s", e)
        return []
...
def save_won_quotes(quotes: list):
    ...
    if len(quotes) > MAX_RECORDS:
        ...
        log.info("LRU eviction: trimmed KB to %d records", MAX_RECORDS)
    with open(WON_QUOTES_FILE, "w") as f:
        json.dump(quotes, f, indent=2, default=str)
    log.debug("Saved %d won quotes to KB", len(quotes))
...
def ingest_scprs_result(
    ...
    if existing_idx is not None:
        ...
        log.debug("KB update: %s $%.2f (PO:%s)", description[:30], unit_price, po_number)
    else:
        quotes.append(record)
        log.debug("KB ingest: %s $%.2f (PO:%s)", description[:30], unit_price, po_number)
