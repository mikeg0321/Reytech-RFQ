...
def fill_ams704(
    ...
    # Supplier info
    # `PERSON PROVIDING QUOTE` is required by Form QA (`form_qa.py:80`) and
    # appears on AMS 704 templates that include the seller-block at the top.
    # Was missing from this mapping, so every PC PDF generated by fill_ams704
    # tripped "Form QA FAIL — Missing: PERSON PROVIDING QUOTE" (visible in
    # Railway logs 2026-04-23 across rev2/3/4 of PC f81c4e9b). Same value as
    # COMPANY REPRESENTATIVE — both refer to the canonical Reytech contact.
    supplier_mappings = [
        ("COMPANY NAME", info.get("company_name", "")),
        ("COMPANY REPRESENTATIVE print name", info.get("representative", "")),
        ("PERSON PROVIDING QUOTE", info.get("representative", "")),
...
def _add_signature_to_pdf(writer, source_pdf_path=None, sig_rect_override=None):
    ...
    try:
        ...
        # Draw date (right portion, vertically centered)
        today = f"{_pst_now().month}/{_pst_now().day}/{_pst_now().year}"
        c.setFont("Helvetica", 9)
        c.setFillColorRGB(0, 0, 0)
        c.drawString(date_x, fb + (fh - 9) / 2, today)
        c.restoreState()
