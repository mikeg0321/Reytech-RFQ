...
def fill_bid_package(input_path, rfq_data, config, output_path):
    ...
    # Resolve the agency profile for this RFQ so the page-trim knows which
    # inline pages have standalone replacements WILL ALSO be generated by
    # the package pipeline. CCHCS required_forms = ["703b","704b","bidpkg",
    # "quote"] — no bidder_decl, no darfur_act → set is empty → trim KEEPS
    # the inline pages (was the PVSP COB-miss 2026-05-13). CalVet +
    # DSH list both → trim SKIPS them. Always-skip rules (CalRecycle SABRC
    ...
    try:
        local_keep = set(_compute_bidpkg_keep_indices(input_path, replaced_by_standalone=_bidpkg_replaced))
...
def _bidpkg_page_skip_reason(page, replaced_by_standalone=frozenset(), page_text=None):
    ...
    `replaced_by_standalone` — set of agency-required-form keys whose
    standalone PDFs WILL BE generated alongside the BidPackage, so the
    matching inline pages can be trimmed. Empty set (the default and
    the CCHCS case) keeps the inline pages intact.

    PR mr-wolf 2026-05-13: this was the canonical CCHCS-COB-miss bug —
    PVSP (CCHCS) `required_forms = ["703b","704b","bidpkg","quote"]`
    generates NO bidder_decl / darfur_act standalones, but the old
    ...
    if _bidder_decl_replaced and any(
            "gspd" in f.lower() or "subcontractor" in f.lower()
            for f in field_names):
        return "GSPD-05-105 Bidder Declaration (use standalone)"
    if (_bidder_decl_replaced
            and any(f.endswith("_105") for f in field_names)
            and any(f.startswith("Text") for f in field_names)):
        return "GSPD-05-105 Bidder Declaration (field pattern _105)"
    # STD 105 Bidder Declaration by field naming convention
    if _bidder_decl_replaced and (
            "solicitation number" in field_sig
            and "subcontractor" in field_sig):
    ...
    if (_darfur_replaced and "10475" in text and "scrutinized" in t
            and "public contract code" in t):
        return "Darfur pg1 (standalone used)"
    if _darfur_replaced and (
            "scrutinized company" in t
            and ("10476" in text or "written permission" in t)):
    ...
    if _bidder_decl_replaced and (
            "gspd" in t
            and ("bidder declaration" in t or "subcontractor" in t)):
        return "GSPD Bidder Declaration text"

    return None  # KEEP


def _compute_bidpkg_keep_indices(template_path, replaced_by_standalone=frozenset()):
    """
    ...
    Cached by (file mtime, replaced_by_standalone); auto-invalidates when template
    is updated AND when the caller's agency profile changes the standalone set.

    `replaced_by_standalone` — passed through to _bidpkg_page_skip_reason so
    the inline pages whose standalones the agency requires get trimmed, and
    those whose standalones the agency does NOT require get kept. The cache
    key includes this set so a CCHCS run (empty set, keeps inline pages) and
    a CalVet run (set has bidder_decl + darfur_act, skips them) don't collide.
    """
    import os as _os
    ...
    _replaced_key = tuple(sorted(replaced_by_standalone))
    _cache_key = (template_path, _replaced_key)
    cached = _BIDPKG_KEEP_CACHE.get(_cache_key)
    ...
    for i, page in enumerate(reader.pages):
        reason = _bidpkg_page_skip_reason(page, replaced_by_standalone=replaced_by_standalone)
        if reason:
            print(f"  BidPkg skip pg{i:02d}: {reason}")
        else:
            keep.append(i)

    _BIDPKG_KEEP_CACHE[_cache_key] = (mtime, keep)
    print(f"  ✓ BidPackage page map: keep indices {keep} (of {len(reader.pages)} total) "
          f"[standalones={list(_replaced_key) or 'none'}]")
    return keep
