...
class FiscalSession:
    ...
    def get_detail(self, results_html, row_index, grid_prefix="ZZ_SCPR_RSLT_VW"):
        action_id = f"{grid_prefix}_CRDMEM_ACCT_NBR${row_index}"
        search_values = {}
    ...
    def _parse_results(self, html):
        ...
        # Strategy 1: ID-based — try both known prefixes
        for prefix in ["ZZ_SCPR_RSLT_VW", "ZZ_SCPR_RD_DVW"]:
            discovered = _discover_grid_ids(soup, prefix)
            if discovered:
                log.info(f"SCPRS found fields with prefix {prefix}: {list(discovered.keys())}")
                break
        
        if not discovered:
            # Broad scan: find any ID ending in $0 containing SCPR
            for el in soup.find_all(id=re.compile(r'SCPR.*\$0$')):
                eid = el.get('id', '')
                m = re.match(r'^(.+?)_(\w+)\$0$', eid)
                if m:
                    prefix = m.group(1)
                    discovered = _discover_grid_ids(soup, prefix)
                    if discovered:
                        log.info(f"SCPRS auto-discovered prefix {prefix}: {list(discovered.keys())}")
                        break
        ...
        if discovered:
            for row_idx in range(total):
                ...
                if "po_number" not in entry:
                    link = soup.find("a", id=f"{prefix}_CRDMEM_ACCT_NBR${row_idx}")
    ...
    try:
        ...
        for c in cands[:5]:
            ...
            if c.get("_results_html"):
                try:
                    gp = c.get("_grid_prefix", "ZZ_SCPR_RSLT_VW")
                    detail = session.get_detail(c["_results_html"], c["_row_index"], gp)
                    time.sleep(0.3)
                except Exception as e:
