...
class FiscalSession:
    ...
    def get_detail(self, results_html, row_index, click_action=None):
        # Use stored action ID if available, otherwise try known patterns
        if not click_action:
            soup = BeautifulSoup(results_html, "html.parser")
            # Find the Nth data row's PO link
            links = []
            for grid in [soup.find("table", id=re.compile(r'SCPR.*\$scroll')),
                         soup.find("table", class_="PSLEVEL1GRIDWBO")]:
                if grid:
                    for a in grid.find_all("a"):
                        aid = a.get("id", "")
                        if "CRDMEM" in aid or "ACCT_NBR" in aid:
                            links.append(aid)
            if row_index < len(links):
                click_action = links[row_index]
            else:
                # Fallback patterns
                for prefix in ["ZZ_SCPR_RSLT_VW", "ZZ_SCPR_RD_DVW"]:
                    click_action = f"{prefix}_CRDMEM_ACCT_NBR${row_index}"
                    if click_action in results_html:
                        break
        
        log.info(f"Detail click action: {click_action}")
    ...
    def _parse_results(self, html):
        ...
        # Column order from FI$Cal SCPRS results grid (confirmed from screenshots):
        # Dept Name | PO# | Assoc POs | First Item Title | Start Date | End Date |
        # Grand Total | Supplier ID | Supplier Name | Cert Type | Acq Type
        COL_MAP = ["dept", "po_number", "associated_pos", "first_item",
                   "start_date", "end_date", "grand_total", "supplier_id",
                   "supplier_name", "cert_type", "acq_type"]

        results = []

        # Strategy 1: Find grid by known scroll table ID
        grid = soup.find("table", id=re.compile(r'SCPR.*\$scroll\$0'))
        if grid:
            log.info(f"Found grid by ID: {grid.get('id')}")
        
        # Strategy 2: Find innermost PSLEVEL1GRID (not wrapper WBO)
        if not grid:
            for t in soup.find_all("table"):
                cls = " ".join(t.get("class", []))
                if cls == "PSLEVEL1GRID" or (
                    "PSLEVEL1GRID" in cls and "WBO" not in cls and "LABEL" not in cls):
                    grid = t
                    log.info(f"Found grid by class: {cls}")
                    break

        # Strategy 3: Find WBO wrapper, get inner table
        if not grid:
            wbo = soup.find("table", class_="PSLEVEL1GRIDWBO")
            if wbo:
                inner = wbo.find("table")
                if inner:
                    grid = inner
                    log.info("Found grid inside GRIDWBO wrapper")

        if not grid:
            log.warning(f"No grid table found despite {total} results")
            return []

        # Walk all rows in the grid
        all_rows = grid.find_all("tr")
        log.info(f"Grid has {len(all_rows)} total rows")
        ...
        for row in all_rows:
            ...
            for i, cell in enumerate(cells):
                if i < len(COL_MAP):
                    ...
                    if text and text != "\xa0":
                        entry[COL_MAP[i]] = text
                        
                    # Check for hyperlink in PO column (PO numbers are clickable)
                    if COL_MAP[i] == "po_number":
                        link = cell.find("a")
                        if link:
                            entry["po_number"] = link.get_text(strip=True)
                            # Extract the PeopleSoft action ID from the link
                            href = link.get("href", "")
                            aid = link.get("id", "")
                            if aid:
                                entry["_click_action"] = aid

            if "po_number" not in entry and len(entry) <= 1:
                continue
            ...
            log.info(f"  Row {idx}: PO={entry.get('po_number','?')} "
                     f"${entry.get('grand_total','?')} {entry.get('supplier_name','?')}")

        log.info(f"Parsed {len(results)} result rows")
        return results
