...
class FiscalSession:
    ...
    def init_session(self):
        ...
        try:
            r1 = self.session.get(f"{SCPRS_SEARCH_URL}?&", timeout=20, allow_redirects=True)
            log.info(f"SCPRS init hit 1: {r1.status_code} ({len(r1.text)} bytes)")

            if "ZZ_SCPRS1_CMP" not in r1.text:
                time.sleep(1)
                r1 = self.session.get(f"{SCPRS_SEARCH_URL}?&", timeout=20, allow_redirects=True)
                log.info(f"SCPRS init hit 2: {r1.status_code} ({len(r1.text)} bytes)")

            self.icsid = self._extract_icsid(r1.text)
            self._last_page_html = r1.text
            if self.icsid:
    ...
    def search(self, description="", from_date="", to_date=""):
        ...
        # Load fresh search page to get valid ICSID
        try:
            r = self.session.get(f"{SCPRS_SEARCH_URL}?&", timeout=20)
            page_html = r.text
            self.icsid = self._extract_icsid(page_html) or self.icsid
        except Exception as e:
...
def test_connection():
    """Test if we can reach FI$Cal SCPRS."""
    if not HAS_SCRAPER:
        return False, "requests/beautifulsoup not installed"
    try:
        s = requests.Session()
        s.headers.update({"User-Agent": USER_AGENT})
        r = s.get(f"{SCPRS_SEARCH_URL}?&", timeout=15, allow_redirects=True)
        if r.status_code == 200 and "SCPRS" in r.text:
            icsid = re.search(r"name='ICSID'[^>]*value='([^']*)'", r.text)
            return True, f"Connected, ICSID={'found' if icsid else 'missing'}"
        return False, f"HTTP {r.status_code}, SCPRS form not found"
