...
|--------|---------|----------|--------|-------------|---------|
| fix/price-history-band-click-target | (auto session) | `C:\Users\mikeg\rfq-price-history-band-click-target` | Active | Surface #14: PRICE HISTORY INTELLIGENCE band on PC detail now whole-band click target (role=button + tabindex + Enter/Space + hover). Was just text + far-right button — looked clickable but wasn't. | 2026-05-04 |
