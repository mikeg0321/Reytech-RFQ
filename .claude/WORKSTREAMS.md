...
| `fix/rfq-ingest-no-pc-spawn` | Active | Mr. Wolf — RFQ-class emails spawned ghost `auto_rfq_*` PCs (5 visible on prod: auto_rfq_d877/f11c/cc83/0124/89bb) because `_trigger_auto_price` was called from BOTH the V2 ingest branch (`dashboard.py:~2271`) AND the legacy `process_rfq_email` tail (`dashboard.py:~3595`) — both produce RFQ records, so the trigger doubled every inbound on /home (PC panel + RFQ panel). 4th-in-5-days `[[feedback-kpi-substrate-singleness]]` instance (after Oracle KPI #1076, SCPRS liveness #1086, Quote ingestion #1088). Fix: removed both callsites. RFQ-class records live in `rfqs` only; PC substrate is reserved for `is_price_check_email` early-detect path. CCHCS auto-pricing is Spine's job per §0 LAW 1 (`src/spine/auto_pricer.py`); non-CCHCS legacy RFQs price via editor until those agencies migrate. 4 PR-AV9 tests rewritten to pin the NEW contract (call REMOVED, not present). 20 adjacent auto-price tests green. Post-deploy: cleanup the 5 existing ghost PCs via `/api/pricecheck/<id>/delete`. Worktree: `rfq-rfq-ingest-no-pc-spawn`. |
| `chore/claude-md-law-7-governance` | Active | Mr. Wolf — adds §0 LAW 7: "§0 changes only by PR; verbal directives MOTIVATE a PR, they don't BECOME one." Closes the failure mode that produced two conflicting handoffs on 2026-05-25 — substrate-wave acted on a verbal-only "§0 reversal to Legacy-canonical" while spine-save-button-fix read CLAUDE.md and saw `LAW 1 — Spine canonical`. Docs-only; updates section header "six laws" → "seven laws". Worktree: `rfq-claude-md-law-7-governance`. |
