...
|--------|---------|----------|--------|-------------|---------|
| refactor/drive-with-retry | (auto session) | `C:\Users\mikeg\rfq-drive-with-retry` | Active | Tier 1d follow-on (handoff `docs/AUDIT_HANDOFF_2026_05_07.md`). Wraps every `service.files()...execute()` in `src/core/gdrive.py` (13 sites) + `MediaIoBaseDownload.next_chunk()` with new `_with_drive_retry` helper that calls into the `with_retry` substrate from PR #833. Predicate covers the gmail transport-layer needles + googleapiclient `HttpError` with status in {429, 500, 502, 503, 504}. Closes the silent-failure path on transient drive errors during nightly backups + observed-send backups. 10 new tests at `tests/test_gdrive_retry.py`. 147/147 critical tests green. | 2026-05-07 |
