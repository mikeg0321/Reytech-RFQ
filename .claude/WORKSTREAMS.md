...
| chore/delete-vestigial-award-queue | (auto session) | `C:\Users\mikeg\Documents\rfq-delete-vestigial-award-queue` | Active | DATA_ARCHITECTURE_MAP §7 silo S7: drop orphan `award_check_queue` table (32 prod rows since 2026-03-23). Removes write block in `agents/post_send_pipeline.on_quote_sent`, CREATE TABLE in `core/db.py`, dead route `/api/v1/quotes/sent-tracker`, tombstone `get_sent_quotes_dashboard()`. Adds migration 29 (`DROP TABLE IF EXISTS`). New `tests/test_post_send_pipeline.py` regression guard. | 2026-04-25 |
