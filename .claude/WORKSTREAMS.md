...
| feat/bundle-5-sent-status | Window 2 | `C:\Users\mikeg\rfq-bundle-5-sent-status` | Active | Bundle-5 (audit A+AA): sent-status hygiene audit across every send/mark-sent/archive path + regression tests + gated backfill script + "Mark as sent manually" button + post-send PC lock. Touches `src/api/modules/routes_rfq.py`, `routes_pricecheck.py`, `src/forms/quote_generator.py` (send-side only, NOT ship-to canonical), templates for RFQ/PC detail. **Stay-out:** quote_generator ship-to block, reytech_filler_v4.py, routes_rfq_gen.py, request_classifier.py, institution_resolver.py, agency_config.py (Window 1 is on Bundle-1/4). | 2026-04-22 |
| feat/bundle-6-linker-pricing-rfq-ui | Window 2 | `C:\Users\mikeg\rfq-bundle-6-linker-pricing-rfq-ui` (pending) | Queued | Bundle-6: linker post-hook copies PC `item['pricing']` → RFQ line items by desc match, idempotent; unified line-item macro between PC/RFQ, MFG# left of Description, RFQ summary stacks Subtotal/Tax/Profit. Pure additive. Starts after Bundle-5 PR-5a armed. | 2026-04-22 |
