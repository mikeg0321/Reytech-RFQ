...
|--------|---------|----------|--------|-------------|---------|
| fix/price-history-band-click-target | (auto session) | `C:\Users\mikeg\rfq-price-history-band-click-target` | Merged PR #728 | Surface #14: PRICE HISTORY INTELLIGENCE band on PC detail now whole-band click target (role=button + tabindex + Enter/Space + hover). Was just text + far-right button — looked clickable but wasn't. | 2026-05-04 |
| fix/ship-to-autofill-facility-registry | (auto session) | `C:\Users\mikeg\rfq-ship-to-autofill-facility-registry` | Merged PR #726 | Surface #15: auto-fill ship_to from facility_registry at ingest. When institution resolves to a known FacilityRecord, populate ship_to from address_line1+address_line2. Buyer's explicit header (>3 chars) still wins. | 2026-05-04 |
| fix/kill-profit-floor | (auto session) | `C:\Users\mikeg\rfq-kill-profit-floor` | Merged PR #724 | Surface #16: kill profit floor entirely. Mike doctrine: "10000 $20 > 5 $100." Deletes _check_profit (pc_qa_agent), _apply_dollar_floor (pricing_oracle_v2), MARKUP_WARN_LOW (cchcs_packet_gate), reytech_config.json profit_floor_* keys, product_catalog min_profit, reytech_filler_v4 low_margin status. + 6 doctrine contract tests. | 2026-05-04 |
| fix/pc-status-flip-on-generate | (auto session) | `C:\Users\mikeg\rfq-pc-status-flip-on-generate` | Active | Surfaces #11+#13: post-generate status flip from "draft"→"completed" so workflow guide shows Generated + action bar exposes Send Quote + Preview 704 (resolves no-persistent-download too). | 2026-05-04 |
