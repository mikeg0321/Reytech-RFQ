...
| fix/scprs-dashboard-sql-syntax | This window | `C:\Users\mikeg\Reytech-RFQ` (main checkout) | Active | P0: `/intel/scprs` empty for 7 weeks — `" + where + "` literal in 4 SQL strings in `scprs_universal_pull.py`. Convert to f-strings + 4 regression tests (shape, filter, empty-db, injection-lock). | 2026-04-23 |
