...
def test_amazon_search(query: str = "nitrile exam gloves") -> dict:
    ...
    try:
        ...
        debug = {
            ...
            "has_robot": "robot" in html.lower(),
            "title": soup.title.get_text(strip=True) if soup.title else "no title",
            "url": resp.url,
        }

        # Count data-asin elements
        all_asins = soup.find_all("div", attrs={"data-asin": True})
        search_results = soup.find_all("div", attrs={"data-asin": True, "data-component-type": "s-search-result"})
        debug["total_asin_divs"] = len(all_asins)
        debug["search_result_divs"] = len(search_results)
