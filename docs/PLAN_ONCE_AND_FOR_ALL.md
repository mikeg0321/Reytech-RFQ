...
| 0.4 | Quote-status race (S4) — operator manual sets get overwritten by background tracker | 🔴 OPEN | **NEW PR:** `mark_quote_status(qid, new, source, expected_prev=None)` in `core/quote_lifecycle_shared.py`; migrate all 4 writers; `WHERE status = expected_prev` makes the update conditional |
| 0.5 | Pre-Phase-1 PC cost reset (the in-flight gap) | 🔴 OPEN | **One-time script** + admin button: `core/admin_tools.py: reset_pc_cost_chips(pc_id)`. Surfaces on PC detail when `cost_source IS NULL`. |
| 0.6 | Phantom flags purged from memory | 🔴 OPEN | Update `project_rfq_session_2026_04_23_complete.md` to drop `ingest.ghost_quarantine_enabled` + `quote.block_unresolved_ship_to` references |
| 0.7 | **Run the oracle backfill against 4 yrs of historical quotes** | 🔴 OPEN — *exists but never connected* | `src/core/oracle_backfill.py:25 backfill_all()` is wired to `POST /api/oracle/backfill-all` (`routes_intel_ops.py:4072`) and tested but no UI surfaces "last run" or priors count. Add `/health/quoting` widget showing `{quotes_won, quotes_lost, calibrations_written, last_run_at}`. Trigger first run in prod. Idempotent per docstring (EMA blending). |
