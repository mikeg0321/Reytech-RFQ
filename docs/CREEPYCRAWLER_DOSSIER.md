...
| O2 | HIGH | **Job #1 (CCHCS migration) is NOT done** per LAW 2. The `cchcs` agency-config was deleted in PR #1157 then **emergency-restored** after it caused a split-brain (Inspector caught it on Duffey `rfq_0124647e`): the legacy `/rfq/<id>/generate` route still calls `match_agency()` + `fill_703b`/`fill_704b`. The retired `src/spine/agency_forms/` renderers are also still present. | `src/core/agency_config.py:181-226`; `src/api/modules/routes_rfq_gen.py:3681-3738`; `src/spine/agency_forms/{std_204,std_1000,dvbe_843,calrecycle_74,cuf,darfur}.py` | Repoint or delete legacy `/rfq/<id>/generate`; then delete the `cchcs` config entry and the `agency_forms/` renderers in a deletion commit visible in `git log`. |
| O3 | HIGH | **Convergence ratchet is flat.** Baseline (`2026-05-21`) = writers **9**, substrates **3**, dirs **2**; unchanged at this sweep. The pack checkpoint **2026-06-20** requires substrate **and** dir counts to DROP or "the pack model has failed." Job #1 deletion PRs are staged, not shipped. | `tests/spine/convergence_baseline.json`; §0 "Pack checkpoint — 2026-06-20" | Land the Job #1 deletion PRs (O2) before 2026-06-15 so the ratchet drops 3→2 substrates; this is a delivery problem, not a correctness one. |
