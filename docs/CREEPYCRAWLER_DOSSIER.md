...
|---|-----|---------|----------|----------------------|
| O1 | HIGH **(needs a product decision — the WRONG handler is live)** | **Shadowed duplicate route handler.** `POST /api/pricecheck/<pcid>/auto-price` is registered **twice in the same file**. Empirically (Werkzeug `MapAdapter.match`), the **live winner is `api_pricecheck_auto_price` @703** — the *simpler, recommendations-only* handler that does **not** persist. The **shadowed/dead** one is `api_pc_auto_price` @5619 — the *race-safe "PR #778 pattern"* (save-lock + catalog→scrape→Amazon fallback + markup), i.e. the **more complete, intended** evolution that **never actually ships**. So this is not a janitorial "delete the dead copy": deleting @703 to let @5619 serve is a real **behavior change** (auto-price would start persisting under a lock with markup). | `routes_pricecheck_admin.py:703` (live) **and** `:5619` (dead, PR #778); both `POST`, same path | **Decision needed from Mike/Architect:** is the intended auto-price behavior the simple recommendations handler (@703) or the race-safe persisting one (@5619)? Then delete the loser and add a `test_no_duplicate_route_rules` guard. Conflict-zone file → sequence the edit. |
...
| O6 | MED → **ADDRESSED (this branch)** | **Python 3.12 is a hard floor with no CI below it.** 5 route modules use f-string-with-backslash (PEP 701, 3.12+ only); they `SyntaxError` and silently fail to load on 3.11 (`dashboard.py` logged "Failed to load route module" and continued — the routes simply vanished). Prod is 3.12 so this was latent, but a 3.11 runner lost whole route modules with no test failure. | `routes_catalog_finance.py:458`, `routes_intel.py:1903`, `routes_orders_full.py:566`, `routes_pricecheck.py:1088`, `routes_voice_contacts.py:568` | **Fixed:** (1) the boot loop now emits a single `CRITICAL` summary naming every dropped module + the running Python version + the `>=3.12` requirement (`dashboard.py`, no longer silent); (2) `.python-version` declares `3.12`; (3) `tests/test_route_modules_compile.py` fails loudly if any `routes_*.py` stops compiling on 3.12 (skips on <3.12 where the SyntaxError is expected). |
