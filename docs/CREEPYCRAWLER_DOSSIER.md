...
|---|-----|---------|----------|----------------------|
| O1 | HIGH → **RESOLVED (this branch)** | **Shadowed duplicate route handler.** `POST /api/pricecheck/<pcid>/auto-price` was registered **twice**. Werkzeug served `api_pricecheck_auto_price` @703 (simple, recommendations-only); the race-safe "PR #778" `api_pc_auto_price` @5619 (save-lock + catalog→scrape→Amazon fallback + markup) was **dead/shadowed** — never reached. So this was not a janitorial delete: the *live* handler was the lesser one. | `routes_pricecheck_admin.py` — was `:703` (live) **and** `:5619` (dead) | **Done:** Mike's call = **keep @703 (current live behavior — no prod change)**. Deleted the dead @5619 handler + its `_api_pc_auto_price_locked` body (`api_pc_auto_price_status` left intact). Added `tests/test_no_duplicate_route_rules.py` (fails CI if any `(path, method)` resolves to >1 endpoint). Live url_map now has 0 duplicate pairs. |
